...
public class BoardController {
	...
	// 메서드로 업무를 준다
	@RequestMapping("/boardList.do") //메서드와 요청 url을 맵핑
	public String boardList( Model model  ) {
		// 매개변수로 필요한 request 등 객체를 받아올 수 있다.
		// Model => 객체 바인딩용 객체
		
		// 게시판 데이터를 가져오기
		// 글번호, 제목, 작성자, 내용, 작성일, 조회수 --> VO
		List<Board> list = mapper.boardList();
		
		
		// 객체바인딩 (request 영역에 list 저장) 
	...
	public String boardContent(Model model, @PathVariable("idx") int idx) {
		// 1. 쿼리스트링을 이용한 데이터 전송
		// 만약 1개만 받을거라면, 매개변수의 == name값 이면, 파라미터 수집할 수 있다.
		...
		// @PathVariable("idx") int num : 변수명과 key값이 다른 경우
		
		
		// view 이름을 리턴X --> 페이지 이동
		// WEB-INF/views/boardContent.jsp
		// ( 메서드 이름 == view의 논리적인 이름 ) 라면, void를 사용해도 페이지가 이동 된다.
		...
		return "boardContent"; // Path Variable을 사용할때는, 반드시 직접 view(return)를 알려줄것
	}
	
	
	// 게시글 삭제할 메서드
	// boardList.do?idx=4&title=스프링&writer=현진
